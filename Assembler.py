...
error_list=[] #list with all errors
...
for i in input_list:
    ...
    if command[0][-1]==":" and command[0][:-1] not in label_list:
        ...
        for j in command[0][:-1]:
            if j not in "_1234567890abcdefghijklmnopqrstuvwxyzQWERTYUIOPASDFGHJKLZXCVBNM":
                error_list.append("Invalid character in label name; line " + str(line_number))
                temp_flag=1
        if temp_flag==0:
        ...
        error_list.append("Label name already exists; line "+str(line_number))
    else:
    ...
    if(command[0])=="var": #if command is declaring a variable
        if(len(command)==2 and command[1] not in var_list):
            ...
            for j in command[1]:
                if j not in "_1234567890abcdefghijklmnopqrstuvwxyzQWERTYUIOPASDFGHJKLZXCVBNM":
                    error_list.append("Invalid character in variable name; line " + str(line_number))
                    temp_flag=1
            if temp_flag==0:
                var_list.append(command[1]) #add the variable to the variable list
        elif (len(command)>2):
            error_list.append("Too many variables declared; line "+str(line_number))
        elif(len(command)==2 and command[1] in var_list):
            error_list.append("Variable name already exists; line "+str(line_number))
        else:
            error_list.append("Variable missing; line "+str(line_number))
...
def opcode_A(list_input,line_number):
    if((list_input[1] or list_input[2] or list_input[3]) not in register_dict):
        error_list.append("Register not defined;"+str(line_number))
        global_error_flag=1
        return
   
    if((list_input[3]=="flags") or (list_input[3]=="FLAGS")):
        error_list.append("Illegal use of flags; line "+str(line_number))
        global_error_flag=1
        return
...
def opcode_B(list_input,line_number):
    if(list_input[1] not in register_dict):
        error_list.append("Register not defined; line "+str(line_number))
        global_error_flag=1
        return
    if ("." in list_input[2][1:]):
        error_list.append("Float values not allowed; line "+str(line_number))
        global_error_flag=1
        return
    if(int(list_input[2][1:])<0 or int(list_input[2][1:])>255):
        error_list.append("Immediate value out of range; line "+str(line_number))
        global_error_flag=1
        return
...
def opcode_D(list_input,line_number):
    ...
    if(list_input[2] not in var_dict):
        error_list.append("Variable not defined; line "+str(line_number))
        global_error_flag=1
        return
...
def opcode_E(list_input,line_number):
    if(list_input[1] not in label_dict):
        error_list.append("Label not defined; line "+str(line_number))
        global_error_flag=1
        return
...
def error_instruction(line_no):
    error_list.append("Command not found; line "+str(line_no))
    return
...
for i in input_list:
    ...
    if(command[0]=="hlt"):
        ...
        if line_number!=len(input_list):
            error_list.append("Halt not used as last instruction; line "+str(line_number))
    ...
    if (command[0]) in opcodeA_list:
        ...
        if len(command)==3:
            ...
            global_error_flag=1
            error_list.append("Invalid input length for Type B Instruction; line " +str(line_number))
        ...
        if len(command)==3:
            ...
            global_error_flag=1
            error_list.append("Invalid input length for Type C Instruction; line " +str(line_number))
        ...
        if len(command)==3:
            ...
            global_error_flag=1
            error_list.append("Invalid input length for Type D Instruction; line " +str(line_number))
        ...
        if len(command)==2:
            ...
            global_error_flag=1
            error_list.append("Invalid input length for Type E Instruction; line " +str(line_number))
        ...
        if len(command)==1:
            ...
            global_error_flag=1
            error_list.append("Invalid input length for Type F Instruction; line " +str(line_number))

    elif (command[0]=="var"):
        if not_var==1:
            error_list.append("Variable not declared at beginning; line " +str(line_number))
    ...
    #checking for the label
        ...
        if(len(command)==0):
            error_list.append("Label incomplete; line " +str(line_number))
        elif command[0] not in instructions_dict:
            error_list.append("Invalid command in label; line " +str(line_number))
...
if found_halt==0:
    error_list.append("Halt instruction missing")
#print(error_list)
#print(out_list)
